...
#$URL='https://data.ecmwf.int/forecasts/20250226/${HH}z/aifs-single/0p25/oper/$YYYY$MM$DD${HH}0000-${FHR3}h-oper-fc';
$URL='https://data.ecmwf.int/forecasts/$YYYY$MM$DD/${HH}z/aifs-single/0p25/oper/$YYYY$MM$DD${HH}0000-${FHR3}h-oper-fc';
...
while ($fhr <= $hr1) {
  # if ($fhr <= 9) { $fhr="0$fhr"; }
   ...
   if ($windows eq 'yes') {
      $err = system("$curl -f --ssl-no-revoke -S $url$inv -o $OUTDIR/$file.tmp");
      $err = $err >> 8;
      if ($err) {
      ...
      open (In, "$curl -f --ssl-no-revoke -s $url$inv |");
   }
   ...
   if ($range ne '') {
      ...
      $err = system("$curl -f --ssl-no-revoke -v -s -r \"$range\" $url$grb -o $OUTDIR/$file.tmp");
      $err = $err >> 8;
      if ($err != 0) {
